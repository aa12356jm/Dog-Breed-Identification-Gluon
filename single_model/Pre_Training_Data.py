...
import os
from model_new import Net, transform_train, transform_test
...
def SaveNd(data, net, name):
    ...
    for fear,label in tqdm(data):
        fear = fear.as_in_context(mx.gpu()) #将数据放在gpu上，取出transform_train函数第一种生成的数据
        #将两种数据分别给两个网络进行训练
        out = net(fear).as_in_context(mx.cpu())
        x.append(out)
        y.append(label)
